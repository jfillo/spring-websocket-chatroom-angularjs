...
public class StompConnectListener implements ApplicationListener<SessionConnectEvent> {
	...
	public void onApplicationEvent(SessionConnectEvent event) {
		...
		String wsSessionId = event.getMessage().getHeaders().get("simpSessionId", String.class);
		String name = event.getMessage().getHeaders().get("simpUser", UsernamePasswordAuthenticationToken.class).getName();
